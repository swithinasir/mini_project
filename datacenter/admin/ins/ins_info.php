...
<br>
	...
	<form action="ins_info.php" method="post" enctype="multipart/form-data" id = "form" onsubmit="return(validate());">
...
function CheckColors(val){
 ...
 if(val=='others')
   ...
   element.style.display='none';
		...
		<div class="row">
			<div class="col-25">
				<label for="fname">Purpose of meeting</label>
			</div>
			<div class="col-25">
			<?php
			$sql="SELECT meet FROM ins_info GROUP BY meet ";
			$result = $con->query($sql);
			?>
			<select name="meet" onchange='CheckColors(this.value);'> 
			<?php
			if ($result->num_rows > 0) 
			{		
				while($row = $result->fetch_assoc()) 
				{
				echo '<option value="'.$row['meet'].'">'.$row['meet'].'</option>';
				} 
			}?>
			...
			<div class="col-25">
			<input type="text" name="meet1" id="meet" style='display:none;'/>
			</div>
		...
		<div class="row">
			...
			<div class="col-25">
				<textarea id="dis" name="dis" required ></textarea>
			</div>
		</div>
		<div class="row">
			<div class="col-25">
...
if(isset($_POST["submit"])){
 ...
 $dis=$_POST["dis"];
	...
	$sql = "INSERT into ins_info (ayear,date,meet,dis,pic) VALUES ('$ayear','$date','$meet','$dis','$final_file')";
	if(mysqli_query($con,$sql))
	{
		
		echo '<script language="javascript">alert("Inserted Successfully");</script>';
		echo '<script type="text/javascript">window.location.href = "ins_info.php";</script>';
	}
	else 
	{
