...
if($cat=='all' || $cat=='dept' || $cat=='in')
{
	if($cat=='all')
	{
		...
		?>
		<div class="sticky" align="right">
			<form action="dinamic2.php" method="post">
			<?php
				foreach($_POST['type'] as $a) {	?>
				<input type="hidden" name="type[]" value="<?php echo $a; ?>">
				<?php }?>
			...
			<form action="dinamic4.php" method="post">
			<?php
				foreach($_POST['type'] as $a) {	?>
				<input type="hidden" name="type[]" value="<?php echo $a; ?>">
				<?php }?>
				...
				<button style="background-color: skyblue;">
					<img width="50px" height="50px" src="../../../images/excel.png">
				</button>
			</form><br>
		</div>		
