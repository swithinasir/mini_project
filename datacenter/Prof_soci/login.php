...
//check if form is submitted
	...
	$result = mysqli_query($con, "SELECT * FROM prof_soci WHERE u_name = '" . $username. "' and pass = '" . $password . "'");
	...
	if(mysqli_num_rows($result)) {
		...
		// $_SESSION['name']=$name;here session is used and value of $username store in $_SESSION.
		 $row = $result->fetch_assoc();
		  ...
		  $_SESSION['name_sci']= $row["name_sci"];
		  $email=$row["email"];
		  $name_in=$row["name_in"];
...
$text_message = "Hello";
$message = "Dear ".$name_in.",<br>      Your Acount for ".$name_sci." of Datacenter ACE have been logged in  ".$date ;
try
{
$mail->IsSMTP(); 
$mail->isHTML(true);
...
if($mail->Send())
{
		 ...
		 header("Location: soci_frame.php");
}
}
...
if(isset($_POST['fp']))
{
    ...
    $result = mysqli_query($con,"SELECT * FROM prof_soci where u_name='$u_name'");
    $row = mysqli_fetch_assoc($result);
	$fetch_user_id=$row['u_name'];
	$email=$row['email'];
	$pass=$row['pass'];
	$name_in=$row['name_in'];
	if($u_name==$fetch_user_id) {
						//php mailer starting
...
$text_message = "Hello";
$message = "Dear ".$name_in.",<br>  The password is <h1>".$pass."</h1>" ;
try
{
$mail->IsSMTP(); 
$mail->isHTML(true);
...
<body>
  ...
  <form class="modal-content animate" action="login.php" method="post">
    ...
    <a href="../index.php">  <span  class="close" title="Home Page">&times;</span> </a>
