...
$sql1="SELECT * FROM dept_info WHERE dept = '$deptid' && year3 IS NOT NULL";
...
if ($result->num_rows > 0) 
{
	...
	<div class="row">
		...
		<div class="col-25">
			<input type="text" id="name" name="name" value="<?php echo $row["year3"] ?>" readonly>
		</div>
	</div>	
	<div class="row">
		<div class="col-25">
		...
		<div class="col-25">
			<input type="text" id="name" name="name" value="<?php echo $row["name3"] ?>" readonly>
		</div>
		<div class="col-25">
			<label for="fname">&nbsp;&nbsp;Branch</label>
		</div>
		<div class="col-25">
			<input type="text" id="branch" name="branch" value="<?php echo $row["branch3"] ?>" readonly>
		</div>
	</div>
	<div id="wrapper"></div>	 
	<div class="row">
	...
	?>
	<form role="form" action="about_deptph.php" method="post" enctype="multipart/form-data">
		<th>
			Academic Year<br>
	...
	$deptid=$_SESSION['deptid'];
	$aaa=$deptid."_dept";
	$sql3="SELECT * FROM `$aaa` WHERE intake3 IS NOT NULL";
	...
	{		
		while($row3 = $result3->fetch_assoc()) 
		{
			...
			<TR>
				...
				<TD>
					<INPUT type="text" name="" value="<?php echo $row3["intake3"] ?>" readonly />
				</TD>
				<TD>
					<INPUT type="text" name="" value="<?php echo $row3["aff3"] ?>" readonly />
				</TD>
				<TD>
					<?php
					if(empty($row3['aff_cer3']))
					{
						$f=1;
						?>
						<center>
							<form action="about_deptph1.php" method="post">
					...
					?>
						<center>
							<a href="../gallery/aff/<?php echo $aaa; ?>/<?php echo $row3['aff_cer3']; ?>" class="button" target="_blank">View</a>
						</center>
					<?php
					}
					?>
				...
				<TD>
					...
					?>
						<center>
							<a href="../gallery/nba/<?php echo $aaa; ?>/<?php echo $row3['nba_cer3']; ?>" class="button" target="_blank">View</a>
						</center>
					<?php
					}
					?>
				...
				<TD>
					...
					<form action="about_deptph.php" method="post">
						<INPUT type="hidden" name="id" value="<?php echo $row3["id"] ?>" />
...
if(isset($_POST["submit"])){
 ...
 if ($result3->num_rows > 0) 
 {
	...
	$id = $row3["id"];
	$sql2 = "UPDATE dept_info SET `year3`= '$year' , `name3`= '$name' , `branch3`= '$branch' WHERE id = '$id'";		

 }
 else
 {
	$sql2 = "INSERT into dept_info (dept,year3,name3,branch3) VALUES ('$deptid','$year','$name','$branch')";	
...
if(isset($_POST["add"])){
 ...
 {	
	$row3 = $result3->fetch_assoc();
	$intake1 = $row3["intake3"];
	if($intake1 == NULL)
	{
		$id = $row3["id"];
	$sql4 = "UPDATE `$aaa` SET `intake3`= '$intake' , `aff3`= '$aff' , `aff_cer3`= '$final_file' , `nba_cer3`= '$final_file1'  WHERE id = '$id'";		
 ...
 {
	$sql4 = "INSERT into `$aaa` (ayear,intake3,aff3,aff_cer3,nba_cer3) VALUES ('$ayear','$intake','$aff','$final_file','$final_file1')";
	if(mysqli_query($con,$sql4))
	{
		echo '<script language="javascript">alert("Inserted Successfully");</script>';
		echo '<script type="text/javascript">window.location.href = "about_deptph.php";</script>';
...
if(isset($_POST["delete"])){
	...
	$deptid=$_SESSION['deptid'];
	$aaa=$deptid."_dept";
	$sql5 = "UPDATE `$aaa` SET `intake3`= NULL , `aff3`= NULL , `aff_cer3`= NULL , `nba_cer3`= NULL  WHERE id = '$id'";
