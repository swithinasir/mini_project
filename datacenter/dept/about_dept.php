...
$sql1="SELECT * FROM dept_info WHERE dept = '$deptid' && year1 IS NOT NULL";
...
if ($result->num_rows > 0) 
{
	...
	<div class="row">
		...
		<div class="col-25">
			<input type="text" id="name" name="name" value="<?php echo $row["year1"] ?>" readonly>
		</div>
	</div>	
	<div class="row">
		<div class="col-25">
		...
		<div class="col-25">
			<input type="text" id="name" name="name" value="<?php echo $row["name1"] ?>" readonly>
		</div>
		<div class="col-25">
			<label for="fname">&nbsp;&nbsp;Branch</label>
		</div>
		<div class="col-25">
			<input type="text" id="branch" name="branch" value="<?php echo $row["branch1"] ?>" readonly>
		</div>
	</div>
	<div id="wrapper"></div>	 
	<div class="row">
	...
	?>
	<form role="form" action="about_dept.php" method="post" enctype="multipart/form-data">
		<th>
			Academic Year<br>
	...
	$deptid=$_SESSION['deptid'];
	$aaa=$deptid."_dept";
	$sql3="SELECT * FROM `$aaa` WHERE intake1 IS NOT NULL";
	...
	{		
		while($row3 = $result3->fetch_assoc()) 
		{
			...
			<TR>
				<TD>
					<INPUT type="text" name="ayear" value="<?php echo $row3["ayear"] ?>"  />
				</TD>
				<TD>
					<INPUT type="text" name="intake1" value="<?php echo $row3["intake1"] ?>"  />
				</TD>
				<TD>
					<INPUT type="text" name="aff1" value="<?php echo $row3["aff1"] ?>"  />
				</TD>
				<TD>
					<?php
					if(empty($row3['aff_cer1']))
					{
						$f=1;
						?>
						<center>
							<form action="about_dept1.php" method="post">
					...
					?>
						<center>
							<a href="../gallery/aff/<?php echo $aaa; ?>/<?php echo $row3['aff_cer1']; ?>" class="button" target="_blank">View</a>
						</center>
					<?php
					}
					?>
				...
				<TD>
					...
					?>
						<center>
							<a href="../gallery/nba/<?php echo $aaa; ?>/<?php echo $row3['nba_cer1']; ?>" class="button" target="_blank">View</a>
						</center>
					<?php
					}
					?>
				...
				<TD>
					...
					<form action="about_dept.php" method="post">
						<INPUT type="hidden" name="id" value="<?php echo $row3["id"] ?>" />
...
if(isset($_POST["submit"])){
 ...
 if ($result3->num_rows > 0) 
 {
	...
	$id = $row3["id"];
	$sql2 = "UPDATE dept_info SET `year1`= '$year' , `name1`= '$name' , `branch1`= '$branch' WHERE id = '$id'";		

 }
 else
 {
	$sql2 = "INSERT into dept_info (dept,year1,name1,branch1) VALUES ('$deptid','$year','$name','$branch')";	
 }	if(mysqli_query($con,$sql2))
	{
		echo '<script language="javascript">alert("Inserted Successfully");</script>';
		echo '<script type="text/javascript">window.location.href = "about_dept.php";</script>';
	}
	else 
	{
...
if(isset($_POST["add"])){
 ...
 {	
	$row3 = $result3->fetch_assoc();
	$intake1 = $row3["intake1"];
	if($intake1 == NULL)
	{
		$id = $row3["id"];
		$sql4 = "UPDATE `$aaa` SET `intake1`= '$intake' , `aff1`= '$aff' , `aff_cer1`= '$final_file' , `nba_cer1`= '$final_file1'  WHERE id = '$id'";
 ...
 {
	$sql4 = "INSERT into `$aaa` (ayear,intake1,aff1,aff_cer1,nba_cer1) VALUES ('$ayear','$intake','$aff','$final_file','$final_file1')";
	if(mysqli_query($con,$sql4))
	{
		echo '<script language="javascript">alert("Inserted Successfully");</script>';
		echo '<script type="text/javascript">window.location.href = "about_dept.php";</script>';
...
if(isset($_POST["delete"])){
	...
	$deptid=$_SESSION['deptid'];
	$aaa=$deptid."_dept";
	$sql5 = "UPDATE `$aaa` SET `intake1`= NULL , `aff1`= NULL ,`aff_cer1`= NULL , `nba_cer1`= NULL  WHERE id = '$id'";
